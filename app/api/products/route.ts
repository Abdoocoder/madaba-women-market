 ...
 * /api/products/{id}:
 *   delete:
 *     description: Deletes a product for the authenticated seller
 *     parameters:
 *       - name: id
 *         in: path
 *         required: true
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
    ...
    try {
        ...
        if (productData.sellerId !== user.id) {
            console.log('❌ Access denied - product does not belong to user');
            return NextResponse.json({ message: 'Access denied - you can only delete your own products' }, { status: 403 });
        }
